...
void Assets::loadFromFile(const std::string &path) {
  ...
  eSoundTypes sound_types; // Define a variable for sound types
  file >> str;
  while (file.good() && str != " ") {
    switch (toascii(str[0])) {
      ...
      case eAssetsTypes::SOUND: {
          std::string name, soundPath;
          file >> name >> soundPath;
          sound_types = static_cast<eSoundTypes>(std::stoi(name));

          if (sound_types < eSoundTypes::SLAST && sound_types > 0)
              addSound(sound_types, soundPath);
          else {
              std::cerr << "unknown sound enumeration " << std::endl;
              getline(file, str);
          }
...
void Assets::addSound(eSoundTypes soundType, const std::string& path) {
    sf::SoundBuffer buffer;
    if (!buffer.loadFromFile(path)) {
        std::cerr << "Could not load sound file: " << path << std::endl;
    }
    else {
        m_soundMap[soundType] = std::move(buffer);
...
const sf::SoundBuffer& Assets::getSound(eSoundTypes soundType) const {
    assert(m_soundMap.find(soundType) != m_soundMap.end());
    return m_soundMap.at(soundType);
}
